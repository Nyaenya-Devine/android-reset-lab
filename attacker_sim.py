...
def attack_brute_force():
    for i in range(4):
        ...
        security_logger.log_event("LOGIN_FAILED", "ops", msg,
                                  severity="WARNING", request_id="ATK-1")


def attack_out_of_hours():
    security_logger.log_event("RESET_REQUESTED", "attacker", "created",
                              device_id="AND-001", request_id="ATK-2",
                              timestamp=NIGHT)


def attack_privilege_escalation(token):
    ok, msg = authorization.authorize(token, "approve_reset")
    security_logger.log_event("ACCESS_DENIED", "attacker", msg,
                              severity="WARNING", request_id="ATK-3")
...
def attack_replay():
    security_logger.log_event("RESET_REQUESTED", "attacker", "created",
                              device_id="AND-002", request_id="ATK-5")
    security_logger.log_event("RESET_REQUESTED", "attacker", "created",
                              device_id="AND-002", request_id="ATK-5")
